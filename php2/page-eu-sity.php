<!DOCTYPE html>
<html lang="ro">
<head>
    <meta charset="UTF-8">
    <title>History</title>
    <link href="../css/style.css" rel="stylesheet">
</head>
<body>
<a href="../index.php">
    <img src="../img/history-title.jpg" class="logoWidth alignLeft" alt="Logotip">
</a>
<img src="../img/banner.jpg" class="bannerWidth alignLeft" alt="Banner">

<nav class="menu">
    <ul>
        <li><a href="../php2/page-eu.php">Uniunea Europeană</a>
            <ul>
                <li><a href="page-eu-ist.php">Istoria Uniunii Europene</a>
                <li><a href="../php2/page-eu-sity.php">10 cele mai mari orașe</a>
            </ul>
        <li><a href="../php2/page-hu.php">Ungaria</a>
            <ul>
                <li><a href="../php2/page-hu-regat.php">Regatul ungar</a>
                <li><a href="../php2/page-hu-sity.php">Orașele principale din Ungaria</a>
            </ul>
        <li><a href="../php2/page-ordin.php">Ordinul Templierilor</a>
            <ul>
                <li><a href="page-ist-temp.php">Istoria Templierilor</a>
            </ul>
    </ul>
</nav>

    <h1 class="h2">10 cele mai mari orașe din Uniunea Europeană</h1>
    <table class="table">
       <tr>
           <th>Loc</th>
           <th>Numele orașului</th>
           <th>Statul</th>
           <th>Populația</th>
       </tr>
       <tr>
           <td>1</td>
           <td>Paris</td>
           <td>Franța</td>
           <td>12.183.893</td>
       </tr>
       <tr>
           <td>2</td>
           <td>Madrid</td>
           <td>Spania</td>
           <td>6.549.520</td>
       </tr>
       <tr>
           <td>3</td>
           <td>Barcelona</td>
           <td>Spania</td>
           <td>5.514.881</td>
       </tr>
       <tr>
           <td>4</td>
           <td>Berlin</td>
           <td>Germania</td>
           <td>5.259.363</td>
       </tr>
       <tr>
           <td>5</td>
           <td>Ruhr</td>
           <td>Germania</td>
           <td>5.113.487</td>
       </tr>
       <tr>
           <td>6</td>
           <td>Roma</td>
           <td>Italia</td>
           <td>4.355.725</td>
       </tr>
       <tr>
           <td>7</td>
           <td>Milano</td>
           <td>Italia</td>
           <td>4.336.121</td>
       </tr>
       <tr>
           <td>8</td>
           <td>Atena</td>
           <td>Grecia</td>
           <td>3.576.590</td>
       </tr>
       <tr>
           <td>9</td>
           <td>Hamburg</td>
           <td>Germania</td>
           <td>3.309.215</td>
       </tr>
       <tr>
           <td>10</td>
           <td>Amsterdam</td>
           <td>Țările de Jos</td>
           <td>3.242.852</td>
       </tr>
    </table>

    <img src="../img/paris.jpg" class="imgWidth0" alt="Paris">
    <img src="../img/madrid.jpg" class="imgWidth0" alt="Madrid">
    <img src="../img/roma.jpg" class="imgWidth0" alt="Roma">
    <img src="../img/berlin.jpg" class="imgWidth11" alt="Berlin">
    <img src="../img/rurh.jpg" class="imgWidth11" alt="Rurh">
    <img src="../img/milano.jpg" class="imgWidth11" alt="Milano">
    <img src="../img/atena.jpg" class="imgWidth11" alt="Atena">
    <img src="../img/hamburg.jpg" class="imgWidth10" alt="Hamburg">

</body>
</html>